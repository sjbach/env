...
(defcustom viper-imm-doc-view-bindings t
  "doc-view-mode bindings bindings."
  :type  'boolean
  :group 'viper-in-more-modes)


(defvar viper-imm-doc-view-mode-vi-map
  (let ((map (make-sparse-keymap)))
    (define-key map "f" 'doc-view-next-page)
    (define-key map "b" 'doc-view-previous-page)
    (define-key map "j" (lambda () (interactive) (image-scroll-up 3)))
    (define-key map "k" (lambda () (interactive) (image-scroll-down 3)))
    (define-key map "l" (lambda () (interactive) (image-forward-hscroll 6)))
    (define-key map "h" (lambda () (interactive) (image-backward-hscroll 6)))
    (define-key map "-" 'doc-view-shrink)
    (define-key map "_" 'doc-view-shrink)
    (define-key map "+" 'doc-view-enlarge)
    (define-key map "=" 'doc-view-enlarge) ; STEVE doesn't work
    (define-key map "q" 'quit-window)
    map))

(when viper-imm-doc-view-bindings
  (viper-modify-major-mode 'doc-view-mode
                           'vi-state
                           viper-imm-doc-view-mode-vi-map))
